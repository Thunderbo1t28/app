...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        current_date =  datetime.strptime("2020-01-13", "%Y-%m-%d").date()  #datetime.now().date()
            ...
            while start < total:
                ...
                for item in response_df.itertuples(index=False):
                    ...
                    if len(sectype)==2:
                        ...
                        #print(sec_id)
                        # Проверяем, что контракт присутствует в словаре
                        if sec_id and sec_id in contracts_to_secid:
                            instrument = contracts_to_secid[sec_id]

                            try:
                                contract = Quote.objects.filter(contract=sec_id).order_by('timestamp').first()
                            except Quote.DoesNotExist:
                                continue
