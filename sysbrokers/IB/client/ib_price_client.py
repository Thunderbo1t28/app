...
class ibPriceClient(ibContractsClient):
    def broker_get_historical_futures_data_for_contract(
        ...
        try:
            ...
            print(f"Can't resolve IB contract {str(contract_object_with_ib_broker_config)}")
            raise missingData

        price_data = self._get_generic_data_for_contract(
            ibcontract,  bar_freq=bar_freq, whatToShow=whatToShow #log=specific_log,
        )
    ...
    def get_ib_ticker_object(
        ...
        try:
            ...
            print(f"Can't find matching IB contract for {str(contract_object_with_ib_data)}")
            raise
...
def _avoid_pacing_violation(
    ...
    while _pause_for_pacing(last_call_datetime):
        if not printed_warning_already:
            ...
            print(f"Pausing {msg} seconds to avoid pacing violation")
