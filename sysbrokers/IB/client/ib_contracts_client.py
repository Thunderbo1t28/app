...
class ibContractsClient(ibClient):
    ...
    def broker_get_single_contract_expiry_date(
        ...
        if futures_contract_with_ib_data.is_spread_contract():
            ...
            print("Can only find expiry for single leg contract!")
        ...
        try:
            ...
            print("Contract is missing can't get expiry")
            raise missingContract
    ...
    def _ib_get_uncached_trading_hours(
        ...
        try:
            ...
            print(f"{str(e)} when getting trading hours from {str(contract_object_with_ib_data)}!")
            raise missingData
    ...
    def ib_get_saved_weekly_trading_hours_for_timezone_of_contract(
        ...
        try:
            ...
            print(f"No time zone ID, can't get trading hours for timezone for {str(contract_object_with_ib_data)}")
            raise missingData
    ...
    def ib_get_timezoneid(self, contract_object_with_ib_data: futuresContract) -> str:
        ...
        try:
            ...
            print(f"{str(e)} when getting time zone from {str(contract_object_with_ib_data)}!")
            raise missingData
    ...
    def _get_all_saved_weekly_trading_hours_from_file(self):
        try:
            ...
            print("Saved trading hours file missing - will use only IB hours")
    ...
    def ib_get_min_tick_size(
        ...
        try:
            ...
            print("Can't get tick size as contract missing")
            raise
        ...
        try:
            ...
            print(f"{str(e)} when getting min tick size from {str(ib_contract_details)}!")
            raise missingContract
    ...
    def ib_get_price_magnifier(
        ...
        try:
            ...
            print("Can't get price magnifier as contract missing")
            raise
        ...
        try:
            ...
            print(f"{str(e)} when getting price magnifier from {str(ib_contract_details)}!")
            raise missingContract
    ...
    def ib_get_contract_details(self, contract_object_with_ib_data: futuresContract):
        ...
        try:
            ...
            print(f"Can't get trading hours as contract is missing")
            raise
    ...
    def _get_vanilla_ib_futures_contract(
        ...
        try:
            ...
            print(f"{str(futures_instrument_with_ib_data)} could not resolve contracts: {exception.args[0]}")

            raise missingContract

        return resolved_contract

    def ib_resolve_unique_contract(self, ibcontract_pattern, pst_logger = None):
        """
        ...
        if len(contract_chain) > 1:
            ...
            print(f"Got multiple contracts for {str(ibcontract_pattern)} when only expected a single contract: Check contract date")
            raise missingContract

        if len(contract_chain) == 0:
            log.warning("Failed to resolve contract %s" % str(ibcontract_pattern))
            print(f"Failed to resolve contract {str(ibcontract_pattern)}")
            raise missingContract
