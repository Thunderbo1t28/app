...
IB_FUTURES_CONFIG_FILE = resolve_path_and_filename_for_package(
    f"{BASEDIR}\\sysbrokers\\IB.config\\ib_config_futures.csv"
)


def read_ib_config_from_file() -> IBconfig: #log: pst_logger = get_logger("")
    try:
        df = pd.read_csv(IB_FUTURES_CONFIG_FILE)
    except Exception as e:
        log.warning("Can't read file %s" % IB_FUTURES_CONFIG_FILE)
        print(f"Can't read file {IB_FUTURES_CONFIG_FILE}")
...
def get_instrument_object_from_config(
    ...
    if config is None:
        try:
            ...
            print(f"Can't get config for instrument {instrument_code} as IB configuration file missing")
    ...
    try:
        ...
        print(f"Instrument {instrument_code} is not in IB configuration file")
...
def get_instrument_code_from_broker_instrument_identity(
    ...
    if len(config_rows) == 0:
        ...
        try:
            ...
            #log.critical(msg)
            print(f"Broker symbol {ib_code} (exchange:{ib_exchange}, valid_exchange:{ib_valid_exchange} multiplier:{ib_multiplier}f) not found in configuration file!")
            raise Exception(msg)
    ...
    if len(config_rows) > 1:
        ...
        #log.critical(msg)
        print(f"Broker symbol {ib_code} ({ib_exchange}, {ib_multiplier}) appears more than once in configuration file!")
        raise Exception(msg)
